    ...
    <div id='companies' className='bg-purple-200 p-5'>
        ...
        <div className='grid grid-cols-2 md:grid-cols-4 lg:grid-cols-4 m-3'>
            {CompanyImage.map((item,index)=>(
                <div key={item.id ?? index} className='m-3 transition-all ease-in-out hover:scale-110'>
                    <img src={item.imageUrl} alt={`Company ${item.id ?? index}`} className="h-[40px] md:h-[50px] object-contain mx-auto" />
                </div>
            ))}
        </div>
        </div>
