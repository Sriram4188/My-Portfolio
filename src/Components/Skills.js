    ...
    <div className='mt-10' id="skills">
        ...
        <div className='flex flex-col md:flex-row  justify-evenly px-8 md:px-0 mt-8 '>
          <div className='grid grid-cols-4 md:grid-cols-4 gap-8  items-center'>
            {/** Deduplicate skills so the same logo isn't shown more than once */}
            {(() => {
              const uniqueSkills = Array.from(new Map(skillsList.map(s => [s.icon, s])).values())
              return uniqueSkills.map((item, index) => (
                <div key={item.icon ?? index} className='w-[60px] transition ease-in-out delay-100 hover:scale-125 bg-purple-50 p-3 rounded-full'>
                  <img src={item.icon} alt={`skill-${index}`} className="w-[40px] md:w-[60px] mx-auto" />
                </div>
              ))
            })()}
                </div>
